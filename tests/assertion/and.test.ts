...
describe("and", () => {
  ...
  test.each([[true], [1], ["string"], [Symbol()], [undefined]])(
  ...
  test.each([[true], [1], ["string"], [Symbol()], [undefined]])(
    "should not throw for arrow function returning %p",
    (value) => {
      const assertion = and(() => value, true);
      expect(() => assertion.evaluate()).not.toThrow();
      expect(assertion.evaluate()).toBeUndefined();
  ...
  test.each([[false], [0], [""], [null]])(
    "should throw for arrow function returning %p",
    (value) => {
      const assertion = and(() => value, true);
      expect(() => assertion.evaluate()).toThrow();
    },
  ...
  test("should return string for nested and", () => {
    const assertion = and(true, and(true, and(true, true)));
    expect(assertion.toString()).toEqual(
      "AND(\n  (boolean true)\n  AND(\n    (boolean true)\n    AND(\n      (boolean true)\n      (boolean true)\n    )\n  )\n)",
    );
  });
